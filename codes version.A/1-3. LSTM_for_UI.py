...
for filename in os.listdir(input_folder):
    if filename.endswith('.feather'):
        ...
        for user_id, group in df.groupby('UserID'):
            ...
            if len(sequence) > 1:  # 원래 코드 (2개 이상)
                user_sequences.append(sequence[:-1])  # 입력 시퀀스
                user_targets.append(sequence[1:])  # 타겟 (다음 시간의 선호 벡터)
            else:  # 단 하나의 데이터만 존재할 경우
                user_sequences.append(sequence)  # 자기 자신을 입력으로 사용
                user_targets.append(sequence)  # 자기 자신을 출력으로 사용
        ...
        print('len(user_predictions_df)',len(user_predictions_df))
